...
type EventSubjectNew struct {
	EventType EventType
	Queue     chan Event
	Observers sync.Map
}

func NewEventSubject_(eventType EventType) *EventSubjectNew {
	return &EventSubjectNew{EventType: eventType, Queue: make(chan Event)}
}

func (subject *EventSubjectNew) AddSubscriber(sub *EventSubscriber) {
	// fmt.Println("AddSubscriber:", sub)
	subject.Observers.Store(sub, struct{}{})
}

func (subject *EventSubjectNew) RemoveSubscriber(sub EventSubscriber) {
	subject.Observers.Delete(sub)
}

func (es *EventSubjectNew) NotifySubscriber(ctx context.Context, ac *AppControler, event *Event) {
